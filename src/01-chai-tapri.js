...
export function chaiTapriRevenue(customers) {
  ...
  const adrakCustomers = Math.floor(customers / 3);
  const cuttingCustomers = customers - adrakCustomers;

  const adrakRevenue = adrakCustomers * 15;
  const cuttingRevenue = cuttingCustomers * 10;
